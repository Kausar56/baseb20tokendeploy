      ...
      {deployedTokens.length === 0 ? (
        ...
        <div className="overflow-x-auto rounded-2xl border border-slate-800 bg-slate-950/40 shadow-xl">
          <table className="w-full text-left border-collapse min-w-[800px]">
            <thead>
              <tr className="border-b border-slate-900 bg-slate-950/80 text-[10px] uppercase font-bold tracking-wider text-slate-400">
                <th className="px-6 py-4 text-xs font-semibold">Token Name</th>
                <th className="px-6 py-4 text-xs font-semibold">Symbol</th>
                <th className="px-6 py-4 text-xs font-semibold">Network</th>
                <th className="px-6 py-4 text-xs font-semibold">Status</th>
                <th className="px-6 py-4 text-xs font-semibold text-center">View</th>
                <th className="px-6 py-4 text-xs font-semibold text-right">Manage</th>
              </tr>
            </thead>
              ...
              {deployedTokens.map((token) => {
                ...
                return (
                  <React.Fragment key={token.id}>
                    <tr 
                      className={`transition-all duration-150 hover:bg-slate-900/20 group cursor-pointer ${
                        isExpanded ? 'bg-slate-900/10' : ''
                      }`}
                      onClick={() => toggleExpand(token.id)}
                    >
                      {/* 1. Token Name (with logo) */}
                        ...
                        <div className="flex items-center space-x-3.5">
                          {token.logoUrl.startsWith('data:image/') ? (
                            <img 
                              src={token.logoUrl} 
                              alt="Logo" 
                              className="h-10 w-10 rounded-xl object-cover border border-slate-800 shadow" 
                            ...
                            } text-xl shadow-md border border-slate-800`}>
                              {token.logoUrl || '🔮'}
                            </div>
                          )}
                          <div>
                            <h3 className="font-display font-bold text-white text-sm sm:text-base">{token.name}</h3>
                            <span className="text-[10px] text-slate-500 font-medium">Class: {token.tokenType} B20</span>
                          </div>
                        </div>
                      </td>
                      ...
                      {/* 2. Symbol */}
                      <td className="px-6 py-4.5">
                        <span className="font-mono text-xs text-slate-300 uppercase font-bold tracking-wider bg-slate-900/60 border border-slate-800 px-2 py-1 rounded">
                          {token.symbol}
                        </span>
                      </td>
                      ...
                      <td className="px-6 py-4.5">
                        {token.chainId === 8453 ? (
                          <span className="inline-flex items-center space-x-1.5 text-[10px] font-bold text-blue-400 bg-blue-950/30 border border-blue-900/20 px-2.5 py-1 rounded-full">
                            <span className="h-1.5 w-1.5 rounded-full bg-blue-400 animate-pulse"></span>
                            <span>Base Mainnet</span>
                          </span>
                        ) : (
                          <span className="inline-flex items-center space-x-1.5 text-[10px] font-bold text-emerald-400 bg-emerald-950/30 border border-emerald-900/20 px-2.5 py-1 rounded-full">
                      ...
                      <td className="px-6 py-4.5 text-center" onClick={(e) => e.stopPropagation()}>
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="inline-flex items-center space-x-1 text-xs font-bold text-blue-400 hover:text-blue-300 transition-colors bg-blue-950/15 border border-blue-900/20 px-3 py-1.5 rounded-lg hover:bg-blue-950/30 shadow-sm"
                    ...
                    {isExpanded && (
                      <tr>
                        <td colSpan={6} className="px-6 py-0 bg-slate-950/50">
                          <AnimatePresence>
                            <motion.div
